...
module.exports = function (gulp, config) {
    ...
    gulp.task('clean:examples', gulp.series(function () { return del([config.example.dist]); }));
    gulp.task('watch:example:scripts', gulp.series(buildExampleScripts(true)));
    gulp.task('build:example:scripts', gulp.series(buildExampleScripts()));

    gulp.task('build:example:files', gulp.series(function () {
        return gulp.src(config.example.files, { cwd: config.example.src, base: config.example.src })
            .pipe(gulp.dest(config.example.dist))
            .pipe(connect.reload());
    }));

    gulp.task('build:example:css', gulp.series(function () {
